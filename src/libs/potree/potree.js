...
Potree.MeasuringTool = class MeasuringTool extends THREE.EventDispatcher{
	...
	startInsertionAnnotation(args = {}){
		...
		let insertionCallback = (e) => {
			if(e.button === THREE.MOUSE.LEFT){
				let viewerAnnotation = this.viewer;
				 this.viewer.inputHandler.startDragging(
				 	measure.spheres[measure.spheres.length - 1]);
				...
				function editarAnnotation() {
					var title = prompt("Titulo da anotação :");
					var descricao = prompt("Descrição da anotação :");

					viewerAnnotation.scene.addAnnotation(measure.points[measure.points.length - 1].position.clone(), {	
						"cameraPosition":cameraPosition ,
						"cameraTarget":cameraTarget ,
						"title": title,
						"description": descricao.toString('utf-8')
					});
				}
				editarAnnotation();
				this.viewer.dispatchEvent({
...
function initToolbar(){
	...
	//ANNOTATIONS
	console.log(viewer.measuringTool)
	elToolbar.append(createToolIcon(
		Potree.resourcePath + "/icons/anotation.svg",
		"[title]Anotação",
		function(){
			$("#menu_annotations").next().slideDown();
			 viewer.measuringTool.startInsertionAnnotation();
		}
	));
		...
		while(stack.length > 0){
			...
			let element = $(`
				<div class="annotation-item" style="margin: 8px 20px">
					...
					<span>
						<a onclick="removerAnnotation('${annotation.position.x}','${annotation.position.y}','${annotation.position.z}','${annotation.title}');"><i class="fa fa-times fa-lg" style="margin-top:5px !important;"aria-hidden="true" ></i> </a>
					</span>
				</div>
			`);
			removerAnnotation = (x,y,z,title)=>{
				for(var i =0; i<viewer.scene.annotations.children.length;i++){
					if(viewer.scene.annotations.children[i].title == title && viewer.scene.annotations.children[i].position.x == x && viewer.scene.annotations.children[i].position.y == y && viewer.scene.annotations.children[i].position.z == z)
						{
							viewer.scene.removeAnnotation(viewer.scene.annotations.children[i]);
						}
				}
				rebuild();
				this.viewer.dispatchEvent({
